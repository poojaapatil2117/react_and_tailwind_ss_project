...
const Caption = ({ children, className }) => {
  return <p className={`${className} text-[15px] font-[500] text-gray-100`}>{children}</p>;
};
...
const CustomNavLink = ({ href, className, isActive, children }) => {
  const linkStyles = "text-[17px] font-medium cursor-pointer list-none hover:text-green transition-all ease-in-out";
  const activeClass = isActive ? "bg-green-100 text-green" : "";
...
const PrimaryButton = ({ children, className }) => {
  return (
    <button type="submit" className={`${className} text-white bg-green font-medium rounded-full text-lg px-16 py-3 hover:bg-primary`}>
      {children}
    </button>
  );
};
...
const ProfileCard = ({ children, className }) => {
  return (
    <div className={`${className} w-12 h-12 bg-green-100 flex items-center justify-center rounded-full`}>
      {children}
    </div>
  );
};
...
export const commonClassNameOfInput = "w-full p-4 text-sm text-gray-900 border border-gray-200 focus:ring-green focus:border-green outli";
